...
const Authentication = () => {
  ...
  const handleCreateAccount = async () => {
    if (newUserName && email && password && password === retypedPassword) {
      ...
      try {
        ...
        setIsLoading(false)
        toast.error('Not able to create an account for you. Please try again later.')
      }
    } else {
      setIsLoading(false)
      toast.error('Please double check that all fields in the form are valid and try again.')
    }
  }
    ...
    if (email && password) {
      ...
      if (validateEmail(email)) {
        try {
          ...
          setIsLoading(false)
          toast.error('Not able to sign you in. Please try again later.')
        }
      } else {
        setIsLoading(false)
  ...
  const handleSignInWithGoogleFirebase = async () => {
    try {
      setIsLoading(true)
      await signInWithGoogle()
    } catch (error) {
      setIsLoading(false)
      toast.error('Not able to continue with Google. Please try again later.')
    }
  }
